...
export function initSorting(columns) {
    return (query, state, action) => {
        ...
        if (action && action.name === 'sort') {
            ...
            const current = action.dataset.order ?? null;
            // определяем следующее состояние по карте переходов
            const next = sortMap[current];
            // записываем новое состояние обратно в data-order
            action.dataset.order = next;
            // если новое состояние активно (asc/desc) — запоминаем
            if (next !== null) {
                order = next;
            };
